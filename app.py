...
def get_stock_data(symbol):
    """
    ดึงข้อมูลหุ้นย้อนหลัง 1 ปี จาก yfinance และแคชข้อมูลไว้
    """
    try:
        ...
        # ไม่มีความจำเป็นต้องลบ Timezone ออกจาก data.index อีกต่อไป
        # เราจะทำให้ datetime calculations เป็น Timezone เดียวกันแทน
        return data
...
if selected_symbol:
    ...
    if stock_data is not None:
        ...
        if data_timezone is None:
            ...
            six_months_ago = today - relativedelta(months=6) # เพิ่มสำหรับแนวโน้ม 6 เดือน
        else:
            # แปลง datetime.datetime.now() ให้เป็น Timezone-aware ใน Timezone เดียวกัน
        ...
        stock_info = {
            ...
            "Current Price": f"{prices.iloc[-1]:,.2f}" if not prices.empty else "N/A", # จัดรูปแบบเป็นทศนิยม 2 ตำแหน่ง
            "Previous Close": f"{prices.iloc[-2]:,.2f}" if len(prices) >= 2 else "N/A",
            "Volume": f"{stock_data['Volume'].iloc[-1]:,.0f}" if not stock_data['Volume'].empty else "N/A", # จัดรูปแบบเป็นจำนวนเต็ม
        }
        ...
        # --- ส่วนเพิ่มเติม: Price Chart (Last 6 Months) ---
        st.subheader("Price Chart (Last 6 Months)")
        if not prices.empty and isinstance(prices.index, pd.DatetimeIndex):
            six_month_data = prices.loc[prices.index >= six_months_ago]
            if not six_month_data.empty:
                st.line_chart(six_month_data)
            else:
                st.info("No data to display 6-month chart.")
        else:
            st.info("Price data not suitable for 6-month chart.")
...
...                 "Date": pd.to_datetime(filtered_prices.index).strftime('%Y-%m-%d'), # แปลงเป็นสตริงเพื่อแสดงผล
...                 "Close Price": [f"{p:,.2f}" for p in history_one_month] # จัดรูปแบบเป็นทศนิยม 2 ตำแหน่ง
...             })
